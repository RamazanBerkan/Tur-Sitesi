...
namespace ArgedeSP.WebUI.Areas.Admin.Controllers
{
    ...
    public class UrunlerController : Controller
    {
        ...
        [HttpPost]
        public async Task<JsonResult> UrunSil(int urunId)
        {
            if (urunId == 0)
            {
            ...
            switch (urunSil_OR.Message)
            {
                ...
                case MesajKodu.UrunSipariseEkli:
                    sonuc.Mesaj = "Ürünün siparişleri var, Silinemez";
                    break;
                default:
        ...
        [HttpPost]
        public async Task<IActionResult> UrunEkle([FromForm]UrunEkle_REQ urunEkle_REQ)
        {
            UrunEkle_RES urunEkle_RES = new UrunEkle_RES()
            {
            ...
            return RedirectToAction("UrunDuzenle", new { urunId = ((Urun)urunEkle_OR.ReturnObject).Id });
        }
        [HttpGet]
        public async Task<IActionResult> UrunDuzenle(int urunId)
        {
            OperationResult urun_OR = await _urunBS.UrunGetirSorgusuz(urunId);
            if (!urun_OR.IsSuccess || urun_OR.ReturnObject == null)
            {
                TempData["Hata"] = "Düzenlemek istediğiniz ürün bulunamadı";
        ...
        [HttpPost]
        public async Task<IActionResult> UrunDuzenle([FromForm]UrunEkle_REQ urunEkle_REQ)
        {
            ...
            return RedirectToAction("UrunDuzenle", new { urunId = urunEkle_REQ.Id });
        }
        [HttpGet]
        ...
        [HttpPost]
        public JsonResult UrunKategorileriGetir()
        {
            var dict = Request.Form.ToDictionary(x => x.Key, x => x.Value.ToString());
        ...
        [HttpGet]
        public IActionResult UrunKategorisiEkle()
        {
        ...
        [HttpPost]
        public async Task<IActionResult> UrunKategorisiEkle([FromForm]UrunKategoriEkle_REQ urunKategoriEkle_REQ)
        {
            if (!ModelState.IsValid)
            {
                return View(new UrunKategoriEkle_RES()
            ...
            if (urunKategoriEkle_OR.IsSuccess)
            {
                ...
                return RedirectToAction("UrunKategorisiEkle");
            }
        ...
        [HttpPost]
        public async Task<JsonResult> UrunKategoriSil(int kategoriId)
        {
            if (kategoriId == 0)
            {
                return Json(new Genel_RES()
        ...
        [HttpGet]
        public async Task<IActionResult> UrunKategoriDuzenle(int kategoriId)
        {
            if (kategoriId == 0)
            {
        ...
        [HttpPost]
        public async Task<IActionResult> UrunKategoriDuzenle([FromForm]UrunKategoriEkle_REQ urunKategoriEkle_REQ)
        {
            if (!ModelState.IsValid)
            {
                return View(new UrunKategoriEkle_RES()
            ...
            if (urunKategoriEkle_OR.IsSuccess)
            {
                ...
                return RedirectToAction("UrunKategoriDuzenle", new { kategoriId = urunKategoriEkle_REQ.Id });
            }
        ...
        [HttpPost]
        public JsonResult UrunDilKategorileri(int dil)
        {
