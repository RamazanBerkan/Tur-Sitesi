...
{
    public class ReferansRepository : GenericRepository<Contracts.Entities.Referans>, IReferansRepository
    {
        private ArgedeSPContext argedeSPContext;
        public ReferansRepository(ArgedeSPContext context)
            : base(context)
        {
        ...
        public VeriListeleme SayfalaAramaIle(int sayfa, int sayfaBoyutu, int id, string ad,string seoUrl)
        {
            try
            {
                IQueryable<Referans> query = argedeSPContext.Referanslar;
                ...
                {
                    query = query.Where(x => x.ReferansAdi.Contains(ad));
                }
